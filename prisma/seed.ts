...
async function main() {
    ...
    if (!fs.existsSync(citiesPath)) {
        ...
        for (let i = 0; i < citiesData.length; i += batchSize) {
            ...
            if ((i / batchSize) % 10 === 0) {
                console.log(
                    `  📊 Progress: ${Math.min(i + batchSize, citiesData.length)}/${citiesData.length} processed...`,
                );
            }
        }
        ...
        if (!fs.existsSync(airportsPath)) {
            ...
            for (let i = 0; i < airportsData.length; i += batchSize) {
                ...
                if ((i / batchSize) % 5 === 0) {
                    console.log(
                        `  📊 Progress: ${Math.min(i + batchSize, airportsData.length)}/${airportsData.length} processed...`,
                    );
                }
            }
