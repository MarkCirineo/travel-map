...
import type { OverviewStats, FrequencyStats, TimeStats, GeoStats, FlightStats } from "@/types";
...
export async function getFrequencyStats(userId: string): Promise<FrequencyStats> {
    const stops = await prisma.tripStop.findMany({
        where: { trip: { userId } },
        include: { location: true },
    });

    const locationCounts = new Map<string, { name: string; type: string; count: number }>();
    for (const stop of stops) {
    ...
    const averageTripLength =
        tripLengths.length > 0
            ? Math.round(tripLengths.reduce((sum, t) => sum + t.days, 0) / tripLengths.length)
            : 0;
    ...
    const mostActiveYear =
        tripsPerYear.length > 0 ? [...tripsPerYear].sort((a, b) => b.count - a.count)[0] : null;
...
export async function getTimeStats(userId: string): Promise<TimeStats> {
    ...
    for (let i = 1; i < datedTrips.length; i++) {
        const gap = differenceInDays(datedTrips[i].startDate!, datedTrips[i - 1].endDate!);
...
export async function getGeoStats(userId: string): Promise<GeoStats> {
    ...
    const worldPercentage =
        totalCountries > 0 ? Math.round((visitedCountryIds.size / totalCountries) * 1000) / 10 : 0;
    ...
    for (const stop of stops) {
        ...
        if (!tripCountries.has(stop.tripId)) tripCountries.set(stop.tripId, new Set());
    ...
    for (const trip of tripsWithDates) {
        ...
        const perCountry =
            tripCountryIds.size > 0 ? Math.round(totalDays / tripCountryIds.size) : 0;
...
export async function getFlightStats(userId: string): Promise<FlightStats> {
    ...
    const departureCounts = new Map<string, { name: string; iata: string; count: number }>();
    const arrivalCounts = new Map<string, { name: string; iata: string; count: number }>();
    const routeCounts = new Map<string, { from: string; to: string; count: number }>();
    const airlines = new Map<string, number>();
    let totalDistance = 0;
    let longestFlight: { from: string; to: string; distance: number } | null = null;
    ...
    const mostCommonRoute = [...routeCounts.values()].sort((a, b) => b.count - a.count)[0] || null;
